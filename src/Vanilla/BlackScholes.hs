...
-}
----------------------------------------------------------------------------------------------------
module Vanilla.BlackScholes where


--import qualified HaskellMath.UnitTest
import qualified Math.Probability
...
callValue stockPrice strikePrice riskFreeRate yearsUntilExpiration volatility dividend =
			...
			(Math.Probability.normsdist d1) * (e ** ((-dividend) * yearsUntilExpiration)) * stockPrice - ((e ** (-riskFreeRate * yearsUntilExpiration))) * strikePrice * (Math.Probability.normsdist d2)
	where
		d1 = (calcD1 stockPrice strikePrice riskFreeRate yearsUntilExpiration volatility dividend)
...
callDelta stockPrice strikePrice riskFreeRate yearsUntilExpiration volatility dividend =
			...
			(e ** ((-dividend) * yearsUntilExpiration)) * (Math.Probability.normsdist d1)
	where
		d1 = (calcD1 stockPrice strikePrice riskFreeRate yearsUntilExpiration volatility dividend)
		e = exp 1
...
callGamma stockPrice strikePrice riskFreeRate yearsUntilExpiration volatility dividend =
			...
			((Math.Probability.normalProbabilityDensity d1) * (e ** ((-dividend) * yearsUntilExpiration)) / (stockPrice * volatility * (sqrt yearsUntilExpiration)))
	where
		d1 = (calcD1 stockPrice strikePrice riskFreeRate yearsUntilExpiration volatility dividend)
		e = exp 1
...
callVega stockPrice strikePrice riskFreeRate yearsUntilExpiration volatility dividend =
			...
			(stockPrice * (sqrt yearsUntilExpiration) * (Math.Probability.normalProbabilityDensity d1) * (e ** ((-dividend) * yearsUntilExpiration)))
	where
		d1 = (calcD1 stockPrice strikePrice riskFreeRate yearsUntilExpiration volatility dividend)
		e = exp 1
...
callTheta stockPrice strikePrice riskFreeRate yearsUntilExpiration volatility dividend =
	...
	where
		...
		d2 = (calcD2 stockPrice strikePrice riskFreeRate yearsUntilExpiration volatility dividend)
		e = exp 1
		expression1 = (stockPrice * (Math.Probability.normalProbabilityDensity d1) * volatility * (e ** ((-dividend) * yearsUntilExpiration)) / (2 * (sqrt yearsUntilExpiration)))
		expression2 = (riskFreeRate * strikePrice * (e ** ((-riskFreeRate) * yearsUntilExpiration)) * (Math.Probability.normsdist d2))
		expression3 = (dividend * stockPrice * (Math.Probability.normsdist d1) * (e ** ((-dividend) * yearsUntilExpiration)))
...
callRho stockPrice strikePrice riskFreeRate yearsUntilExpiration volatility dividend =
		...
		(strikePrice * yearsUntilExpiration * (e ** ((-riskFreeRate) * yearsUntilExpiration)) * (Math.Probability.normsdist d2))
...
putDelta stockPrice strikePrice riskFreeRate yearsUntilExpiration volatility dividend =
			...
			(e ** ((-dividend) * yearsUntilExpiration)) * ((Math.Probability.normsdist d1) - 1)
	where
		d1 = (calcD1 stockPrice strikePrice riskFreeRate yearsUntilExpiration volatility dividend)
		e = exp 1
